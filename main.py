...
class Main(QtWidgets.QWidget, Ui_Form):
    ...
    def adjust_text_sizes(self):
        for label in [self.label_2, self.label_3, self.label_4,
                      self.label_5, self.label_6, self.label_rascr, self.label_7]:
            label.adjustSize()
        ...
        try:
            with self.db_manager as db:
                ...
                SELECT o.id, o.status, 
                       ...
                       SUM(oc.total_sale_price) as total_price
                FROM order_request o
                LEFT JOIN customer c ON o.customer_id = c.id
                LEFT JOIN employee e ON o.employee_id = e.id
                LEFT JOIN order_composition oc ON o.id = oc.order_id
                ...
                for order in orders:
                    btn_text = (f"Заказ #{order['id']} | {order['organization_name']} | "
                                f"Статус: {order['status']} | Сумма: {order['total_price']} руб")
    ...
    def show_order_info(self, order):
        try:
            ...
            with self.db_manager as db:
                ...
                INNER JOIN product_materials pm ON sc.id = pm.supply_composition_id
                INNER JOIN order_composition oc ON pm.order_composition_id = oc.id 
                WHERE oc.order_id = %s AND sc.status = 'Доставлено'
                """
                fabric_data = db.execute_query(fabric_query, (order['id'],))
                total_fabric = sum(f['quantity'] for f in fabric_data) if fabric_data else 0
                self.label_3.setText(f"Доступно полотен ткани: {total_fabric}")
                self.label_3.adjustSize()

                order_query = """
                SELECT oc.id, p.name, oc.quantity, oc.width, oc.length, oc.total_sale_price
                ...
                total_price = sum(item['total_sale_price'] for item in order_items)
                self.label_7.setText(f"Общая стоимость заказа: {total_price} руб")
    ...
    def calculate_cutting(self):
        ...
        try:
            ...
            with self.db_manager as db:
                fabric_query = """
                SELECT sc.id, sc.width, sc.length, sc.quantity 
                FROM supply_composition sc
                INNER JOIN product_materials pm ON sc.id = pm.supply_composition_id
                INNER JOIN order_composition oc ON pm.order_composition_id = oc.id 
                WHERE oc.order_id = %s AND sc.status = 'Доставлено'
                """
                fabrics = db.execute_query(fabric_query, (self.current_order['id'],))
            ...
            if not fabrics:
                self.label_6.setText("Нет доступной ткани для этого заказа!")
            ...
            total_fabric = sum(f['quantity'] for f in fabrics)
            used_fabrics = 0
            produced_items = {item['name']: 0 for item in items}
            remaining_items = [item.copy() for item in items]

            for fabric in fabrics:
                fabric_id = fabric['id']
                fabric_width = float(fabric['width'])
                fabric_height = float(fabric['length'])
                fabric_count = int(fabric['quantity'])

                for _ in range(fabric_count):
                    if used_fabrics >= total_fabric or all(item['quantity'] <= 0 for item in remaining_items):
                        break
                        ...
                        for u in used:
                            for item in remaining_items:
                                if item['name'] == u['name']:
                                    ...
                                    produced_items[item['name']] += u['count']
                                    break

            result_text = "Результаты производства:\n"
            for name, count in produced_items.items():
                required = next(i['quantity'] for i in items if i['name'] == name)
                result_text += f"{name}: {count} из {required}\n"
                if count < required:
                    result_text += f"Не хватает ткани для: {name} ({required - count} шт)\n"
    ...
    def create_cutting_map(self, fabric_id, width, height, placements):
        fig = Figure(figsize=(6, 4))
        canvas = FigureCanvas(fig)
        canvas.setFixedSize(600, 400)

        ax = fig.add_subplot(111)
        ax.set_title(f"Полотно #{fabric_id} ({width}x{height} см)")
        ax.set_xlim(0, width)
